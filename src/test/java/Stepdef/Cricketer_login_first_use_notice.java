...
public class Cricketer_login_first_use_notice {
	//INSTANTIATE WEBDRIVER
		WebDriver driver_Cricketer_login_first_use_notice;
		
		@Given("I am a registered user of Cricketer and I open a browser")
		@Test(priority=6)
		@Parameters("browser")
	public void i_am_a_registered_user_of_Cricketer_and_I_open_a_browser(String browser) {
			//If browser is firefox
			if(browser.equalsIgnoreCase("firefox")) {
				driver_Cricketer_login_first_use_notice = new FirefoxDriver();	
			}
		
		//If browser is safari	
			else if (browser.equalsIgnoreCase("safari")) { 
				driver_Cricketer_login_first_use_notice= new SafariDriver();
				driver_Cricketer_login_first_use_notice.manage().window().maximize();
			} 
			
		//If browser is chrome	
			else if (browser.equalsIgnoreCase("chrome")) { 
				driver_Cricketer_login_first_use_notice= new ChromeDriver();
			} 
	}

	@Given("I navigate to a premium article on Cricketer")
	@Test(priority=7)
	public void i_navigate_to_a_premium_article_on_Cricketer() throws IOException, InterruptedException {
		//open a premium article on reaction	
		driver_Cricketer_login_first_use_notice.get("https://www.thecricketer.com/Topics/features/the_summer_before_the_deluge_of_war.html");
				Thread.sleep(10000);
				
			//verify that the premium article is opened	
				String reaction_navigation= driver_Cricketer_login_first_use_notice.getCurrentUrl();
			    try
				{
			    	AssertJUnit.assertTrue(reaction_navigation.contains("https://www.thecricketer.com/Topics/features/the_summer_before_the_deluge_of_war.html"));
			...
			//scroll for first use notice
			  	JavascriptExecutor js = (JavascriptExecutor)driver_Cricketer_login_first_use_notice;
			  	js.executeScript("window.scrollBy(0,500)");    
			
			//Screenshot 1 of reaction first use notice
				File scrFile = ((TakesScreenshot)driver_Cricketer_login_first_use_notice).getScreenshotAs(OutputType.FILE);
			...
			//Screenshot 2 of reaction first use notice
				File scrFile1 = ((TakesScreenshot)driver_Cricketer_login_first_use_notice).getScreenshotAs(OutputType.FILE);
				String timestamp1 = new SimpleDateFormat("yyyy_MM_dd_HH_mm_ss").format(Calendar.getInstance().getTime());
				FileUtils.copyFile(scrFile1, new File("/Users/jay/Desktop/cricketer/"+"_"+timestamp1+"_"+"jpg" ));	
	}

	@When("I click on Login from Cricketer First use notice")
	@Test(priority=8)
	public void i_click_on_Login_from_Cricketer_First_use_notice() throws InterruptedException {
		//call first use notice elements of reaction
				Thread.sleep(15000);
				Cricketer_first_use_notice_Elements Cric_first_use_register1 = new Cricketer_first_use_notice_Elements(driver_Cricketer_login_first_use_notice);
	...
	@Test(priority=9)
	public void i_verify_that_the_Login_Button_Navigates_me_to_the_login_page_to_log_me_in_to_Cricketer() throws IOException, InterruptedException {
		//get screen shot of the login page	
			...
			//get current url of login page
				String actual_click_on_login_from_reaction_FUN= driver_Cricketer_login_first_use_notice.getCurrentUrl();
	...
	@Then("entering the correct email and password logs me in to Cricketer")
	@Test(priority=10)
	public void entering_the_correct_email_and_password_logs_me_in_to_Cricketer() throws IOException, InterruptedException {
		Login_Page_Elements Login_Pop_First_Use = new Login_Page_Elements(driver_Cricketer_login_first_use_notice);
		Thread.sleep(2000);
	...
	@Then("verify that I am being navigated to the same article on cricketer")
	@Test(priority=11)
	public void verify_that_I_am_being_navigated_to_the_same_article_on_cricketer() throws IOException, InterruptedException {
		//scroll for first use notice
	  	JavascriptExecutor js2 = (JavascriptExecutor)driver_Cricketer_login_first_use_notice;
	  	js2.executeScript("window.scrollBy(0,800)"); 
		
		
	//click ok on authorise charge notice
		Thread.sleep(10000);
		Authorise_charge_notice_popbitch authy_popbitch = new Authorise_charge_notice_popbitch(driver_Cricketer_login_first_use_notice);
		authy_popbitch.authorise_charge_notice_click_continue();
		Thread.sleep(3000);
		
		
		//Click on the green tab and 
				PopbitchFirstUseNoticeElements pop_first_use_top_up_from_wallet2 = new PopbitchFirstUseNoticeElements(driver_Cricketer_login_first_use_notice);
				pop_first_use_top_up_from_wallet2.click_on_green_tab();
						
	//check if the wallket is being deducted by 30 p
		Popbitch_Wallet_Elements_staging wallet_elements_1 = new Popbitch_Wallet_Elements_staging(driver_Cricketer_login_first_use_notice);
	...
	//String screenshot_name_3= "Popbitch_full_article_appears";
		...
		File scrFile_3 = ((TakesScreenshot)driver_Cricketer_login_first_use_notice).getScreenshotAs(OutputType.FILE);		
	...
	//String screenshot_name_4= "Popbitch_full_article_appears_2";
		String timestamp_4 = new SimpleDateFormat("yyyy_MM_dd_HH_mm_ss").format(Calendar.getInstance().getTime());
						File scrFile_4 = ((TakesScreenshot)driver_Cricketer_login_first_use_notice).getScreenshotAs(OutputType.FILE);		
						FileUtils.copyFile(scrFile_4, new File("/Users/jay/Desktop/cricketer/"+"_"+timestamp_4+"_"+"jpg" ));		
						Thread.sleep(3000);		
			
						JavascriptExecutor js1 = (JavascriptExecutor)driver_Cricketer_login_first_use_notice;
						js1.executeScript("window.scrollBy(0,500)");		
						String timestamp_5 = new SimpleDateFormat("yyyy_MM_dd_HH_mm_ss").format(Calendar.getInstance().getTime());
						File scrFile_5 = ((TakesScreenshot)driver_Cricketer_login_first_use_notice).getScreenshotAs(OutputType.FILE);		
					...
					//Verify the user landed on the same article
						String Login_From_PopBitch_Frist_Use_Notice_Current_Url1= driver_Cricketer_login_first_use_notice.getCurrentUrl();	   
					    String Login_From_PopBitch_Frist_Use_Notice_Popbitch_Login_Url1 = "https://www.thecricketer.com/Topics/features/the_summer_before_the_deluge_of_war.html";
